...
export default function makeAdminsDb({ db }: { db: Firestore }): DatabaseEntity<Admin> {
    ...
    async function add(adminInfo: Required<Admin>) {
        const result = await db.collection(CollectionPaths.ADMIN).doc().set(adminInfo);
    ...
    async function findAll({ name }: { name?: string }) {
        ...
        const result = name ? await db.collection(CollectionPaths.ADMIN)
            .where(...opts)
            .get() : await db.collection(CollectionPaths.ADMIN).get();
    ...
    async function update({ id, data }: { id: string, data: Required<Admin> }) {
