...
export default function makeTenantsDb({
  db,
}: {
  db: Firestore;
}): DatabaseEntity<Tenant> {
  ...
  async function findAll({
    ...
    name?: string;
  }): Promise<DatabaseFunction<Required<Tenant>[]> & { _name: string }> {
    ...
    const result = name
      ? await db
          ...
          .where(...opts)
          .get()
      : await db.collection(CollectionPaths.TENANT).get();
